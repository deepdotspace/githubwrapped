...
// On an unexpected throw, return a clean envelope. The real error/stack is only
// exposed behind ?__debug=1 (never leaked to normal callers).
app.onError((err, c) => {
  const url = new URL(c.req.url)
  if (url.pathname.startsWith('/api/')) {
    if (c.env.ALLOW_DEBUG_ROUTES === 'true' && url.searchParams.get('__debug') === '1') {
      return c.json(
        {
          error: 'server',
    ...
    return c.json({ error: 'Something went wrong on our end.', code: 'UPSTREAM' }, 500)
  }
